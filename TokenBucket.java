...
public class TokenBucket {
    ...
    final int power = 2;
    final double rate = Math.pow(2d,power);// 注入速度
    ...
    private void inject(){
        ...
        currCapacity = Math.min((int)rate * (timeInterval << power) + currCapacity , totalCapacity);
        System.out.println("原来剩余："+surplusCapacity+"，本次注入了："+(int)rate * (timeInterval << power)+"，注入之后的容量为："+currCapacity);
    }
